...
json.forEach((item) => {
  ...
  //   img.src = 'Another/img/default.png'; // Fallback image
  // });

  const overlayIntro = document.createElement("div");
  overlayIntro.style.position = "fixed";
  overlayIntro.style.top = "0";
  overlayIntro.style.left = "0";
  overlayIntro.style.width = "100vw";
  overlayIntro.style.height = "100vh";
  overlayIntro.style.backgroundColor = "black";
  overlayIntro.style.display = "flex";
  overlayIntro.style.justifyContent = "center";
  overlayIntro.style.alignItems = "center";
  overlayIntro.style.zIndex = "2000";
  overlayIntro.style.transition = "opacity 1s";
  overlayIntro.style.opacity = "1";

  const portfolioTitle = document.createElement("h1");
  portfolioTitle.textContent = "";
  portfolioTitle.style.fontSize = "150px"; // Adjust the size as needed
  portfolioTitle.style.color = "white";
  portfolioTitle.style.fontFamily = "'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif";
  portfolioTitle.style.letterSpacing = "0.1em";
  portfolioTitle.style.textAlign = "center";
  portfolioTitle.style.position = "absolute";
  portfolioTitle.style.top = "40%";
  portfolioTitle.style.left = "50%";
  portfolioTitle.style.transform = "translate(-50%, -50%)";
  portfolioTitle.style.margin = "0";
  portfolioTitle.style.zIndex = "2100";
  overlayIntro.appendChild(portfolioTitle);

  const typewriterPortfolio = (element, text, speed = 50) => {
    let index = 0;
    ...
    }, speed * 6); // Double le délai pour ralentir la vitesse
    
  };
  

  typewriterPortfolio(portfolioTitle, "PORTFOLIO");

  document.body.appendChild(overlayIntro);
...
// Add typewriter effect to the h2 in the header
const headerH2 = document.querySelector("header h2");
const typewriterEffectH2 = (element, text, speed = 50) => {
  let index = 0;
  const interval = setInterval(() => {
    if (index < text.length) {
      element.textContent += text[index];
...
// Clear the text content and apply the typewriter effect
const h2Text = headerH2.textContent;
headerH2.textContent = "";
typewriterEffectH2(headerH2, h2Text);
...
headerH3.addEventListener("click", () => {
  ...
  // Add typewriter effect to the text inside the rectangle
  ...
  // Clear the text content and apply the typewriter effect
  text.textContent = "";
  typewriterEffect(text, "Bonjour, je m'appelle Manon Lenne et je suis actuellement étudiante en 2ème année à l'ESAD d'Amiens.\nPassionnée par l'art de manière générale, j'ai choisi de me spécialiser dans le design graphique.\nJe suis particulièrement intéressée par la création d'affiches et la mise en page.\nJ'aime également explorer d'autres domaines tels que le dessin, la photographie et la mode.");
